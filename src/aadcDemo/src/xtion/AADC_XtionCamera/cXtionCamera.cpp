...
ADTF_FILTER_PLUGIN("Xtion Capture Device", OID_ADTF_XTION_CAPTURE, cXtionCamera)
...
tResult cXtionCamera::Start(ucom::IException** __exception_ptr)
{        
...
tResult cXtionCamera::Stop(ucom::IException** __exception_ptr)
{
    if (m_oWorker.GetState() == cKernelThread::TS_Running)
        {    
...
tResult cXtionCamera::Shutdown(tInitStage eStage, ucom::IException** __exception_ptr)
{
...
tResult cXtionCamera::ThreadFunc(cKernelThread* pThread, tVoid* pvUserData, tSize szUserData)
{
...
tResult cXtionCamera::LoadConfiguration()
{
    ...
    if (cFileSystem::Exists(m_fileConfig))
    {
        ...
        // load setting for general configuration of xtion        
            ...
            { 
                ...
                if (IS_OK((*itElem)->FindNode("setAutoExposure", pConfigElement)))
                {
                     m_setAutoExposure = (pConfigElement->GetData()).AsBool();
                }
                if (IS_OK((*itElem)->FindNode("setAutoWhiteBalance", pConfigElement)))
                {
                     m_setAutoWhiteBalance = (pConfigElement->GetData()).AsBool();
                }   
            }
        }
        ...
        if (oElems.size() > 0)
        {
            ...
            LOG_INFO(cString::Format("Xtion Device: Options: setDepthColorSync: %d, setRegistration: %d, setAutoExposure %d, setAutoWhiteBalance %d",m_setDepthColorSync,m_setRegistration,m_setAutoExposure,m_setAutoWhiteBalance));    
        }
    }
    else
    {
